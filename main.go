...
import (
	...
	// receiver "github.com/logzio/lastpass-logs-receiver"
...
func createLastPassReceiver() (lastPassLogsReceiver, error) {
	...
	rec, err := newLastPassLogsReceiver(
		enterpriseUrl,
...
func (sfc *lastPassCollector) collect() {
	...
	logsToSend := sfc.receiver.getLogs(lastPassApiKey)
	for _, log := range logsToSend {

	}
	dataLastTime := []byte(logsToSend[0].Time)
